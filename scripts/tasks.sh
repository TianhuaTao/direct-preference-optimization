...
##################
# pythia ultrafeedback sft >> running
...
# pythia ultrafeedback dpo (skip sft)
...
python -u train.py model=olmo-7b datasets=[ultrafeedback] loss=sft exp_name=ultrafeedback_sft_olmo gradient_accumulation_steps=2 batch_size=64 eval_batch_size=32 trainer=FSDPTrainer sample_during_eval=false model.fsdp_policy_mp=bfloat16 eval_every=12800

# OLMo ultrafeedback dpo
python -u train.py model=olmo-7b datasets=[ultrafeedback] loss=dpo loss.beta=0.1 exp_name=ultrafeedback_dpo_olmo gradient_accumulation_steps=4 batch_size=64 eval_batch_size=32 trainer=FSDPTrainer sample_during_eval=false model.fsdp_policy_mp=bfloat16 model.archive=??/LATEST/policy.pt eval_every=12800 model.reference_dtype=bfloat16
